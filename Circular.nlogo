...
to create-ants
  if(ticks mod 650 = 0 and ticks > 1 and number-of-ants < 5)[
    set number-of-ants number-of-ants + 1
    setup-ants
  ]
end
...
to go
   ...
   ask formigas [
     ...
     ifelse(sentido = "close")[
        ifelse(patch-ahead 1 != nobody and not member? u-interno [cores] of patch-ahead 1)
        [ fd 1
          set pcolor u-interno
          ask patch-here [ set cores lput pcolor cores ]
        ]
        [ ifelse(all? neighbors4 [pcolor = cor-interna])
          [ set u-interno u-interno - 1
            set sentido "open"
            set pcolor u-interno
            ask patch-here [ set cores lput pcolor cores ]
          ]
     ...
     [ ifelse(patch-right-and-ahead 90 1 != nobody and not member? u-interno [cores] of patch-right-and-ahead 90 1)
       [ set heading heading + 90
         set turn-side turn-side + 1
         fd 1
         set pcolor u-interno
