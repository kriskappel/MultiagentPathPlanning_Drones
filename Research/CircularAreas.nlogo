...
to go
   ...
   ask formigas [
     ...
     ifelse(sentido = "close")[
        ifelse(patch-ahead 1 != nobody and [u-value] of patch-ahead 1 != u-interno and limit)
        [ fd 1 set front-steps front-steps + 1]
        [ ifelse(all? neighbors4 [u-value = [u-interno] of myself])
          [ set u-interno u-interno + 1 set sentido "open"]
          [
     ...
     ; detecta patch do lado direito disponivel
     ; se estiver move pra lá
     ; senao anda pra frente

     [ ifelse(patch-right-and-ahead 90 1 != nobody and limit and [u-value] of patch-right-and-ahead 90 1 != u-interno )
       [
         set heading heading + 90
         fd 1
         set turn-side turn-side + 1
       ...
       [ ifelse(all? neighbors4 [u-value = [u-interno] of myself])
         [ set u-interno u-interno + 1 set sentido "close"  ]
         [ fd 1 set front-steps front-steps + 1]
       ]
     ]
...
to-report limit
  ...
  [
    ...
    [ ifelse ([pycor] of patch-ahead 1 = limity) or ([pxcor] of patch-ahead 1 = limitx)
      [report false]
      [report true]
    ]
    [report false]
