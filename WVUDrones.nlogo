...
to setup-ants
  create-formigas number-of-ants[
    ...
    while [i <= 50] ;inicializa e preenche com 0 a lista de curvas de cada agente
    [
...
to go
   while [ ticks <= 10000 ][
     ...
     ask formigas [
       ...
       delete-fire
       let neighborMin [u-value] of min-one-of neighbors4 [u-value]; escolhe o vertice com menor u-value dos ortogonais a ele
       ifelse patch-ahead 1 = nobody ;testa se o proximo patch é valido

       [
         add1-curve-list ;adiciona 1 na lista de curvas

         set sucessor min-one-of neighbors4 [u-value];caso não for anda para o menor vizinho dos 4
         facexy ([pxcor] of sucessor) ([pycor] of sucessor)
        ; Atualizando o u-value do patch atual ou nao
        ; se u(s) <= u(succ(s, a)) entao u(s) := 1 + u(s)

        ; Se o u-value da celula atual for menor ou igual ao u-value da celula destino
        ; entao o u-value da celula atual eh atualizado com o valor da propria celula mais um.
        ; Caso a condição não seja atendida o valor da celula atual permanece o mesmo.

         let novo-valor [u-value] of sucessor

         ask patch-here [
           if(u-value <= novo-valor)[
       ...
       [
         let prox [u-value] of patch-ahead 1; se o patch logo a frente for valido ele testa os 2

         ifelse (prox = neighborMin) ;se o patch logo a frente for um dos possiveis minimos ele escolhe ir pra frente
         [
...
to set-file-name
  ...
  [
    let i 0
    set file (word "../csvresults/LRTAResults" i ".csv")
...
930
101
The default file name will be WVUResults.csv
12
0.0
1
...
@#$#@#$#@
<experiments>
  <experiment name="Wagner Value Update Rule" repetitions="2" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
