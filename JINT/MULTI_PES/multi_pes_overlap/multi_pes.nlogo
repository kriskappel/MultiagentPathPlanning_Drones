...
to setup-ants
  create-formigas 1[
    ...
    set turtle_pos n-values number-of-ants [0]
  ]
...
to go
     ...
     ask formigas [
       ...
       [
         if (i != who)
         [
           if (is-patch? item i turtle_pos)
           [
             let returned_list simulate_ants item i map-list item i turtle_pos item i turtle_head i map-list
             set map-list replace-item i map-list (item 0 returned_list)
             set turtle_pos replace-item i turtle_pos (item 1 returned_list)
             set turtle_head replace-item i turtle_head (item 2 returned_list)
           ]
         ]
...
;         print item 1 turtle_pos
       ...
       set turtle_pos neighborPos
       set turtle_head neighborHead]
...
to-report simulate_ants [matrix location orientation number list_matrix]

  let uav-number number
  let uav-matrix matrix:copy matrix
  let uav-return matrix:copy matrix
  ...
  [
    if(uav-number != i)
    [
      set uav-matrix matrix:plus uav-matrix item i list_matrix
  ...
  [report (list uav-return uav-pos uav-heading)];se nao existe nenhum vizinho possivel ele só retorna o patch atual
  ...
  [; aqui se o menor for o logo a frente dai retorna ele, caso contrario continua a execução
    ...
    [
      set uav-return matrix:set-and-report uav-return ([pycor] of ahead) ([pxcor] of ahead) ( matrix:get uav-return ([pycor] of ahead) ([pxcor] of ahead) + 1)

      report (list uav-return  ahead  uav-heading)
  ...
  [; se os dois  forem possiveis
    ...
    [; e um deles for o menor e os dois forem iguais dai ele passa um random e retorna
      ...
      [
        ...
        if rng = side90
        [
          set uav-return matrix:set-and-report uav-return ([pycor] of side90) ([pxcor] of side90) ( matrix:get uav-return ([pycor] of side90) ([pxcor] of side90) + 1)

          report (list uav-return side90 (uav-heading + 90))
        ]

        if rng = sideM90
        [
          set uav-return matrix:set-and-report uav-return ([pycor] of sideM90) ([pxcor] of sideM90) ( matrix:get uav-return ([pycor] of sideM90) ([pxcor] of sideM90) + 1)

          report (list uav-return sideM90 (uav-heading - 90))
        ]
  ...
  [
    set uav-return matrix:set-and-report uav-return ([pycor] of behind) ([pxcor] of behind) ( matrix:get uav-return ([pycor] of behind) ([pxcor] of behind) + 1)

    report (list uav-return behind (uav-heading + 180))
  ]
end
@#$#@#$#@
GRAPHICS-WINDOW
...
<experiments>
  ...
  <experiment name="multi_pes10k" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="multi_pes15k" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="multi_pes20k" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
  ...
  <experiment name="MM40k" repetitions="30" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
